...
sPayMethod = Request("PAYMETHOD")
'If sPayMethod = "CASH" Or sPayMethod = "CHECK" Then
	dTip = 0
'Else
'	dTip = Request("TIP")
'End If
...
If GetStoreInfo(nStore, sName, sAddress1, sAddress2, sCity, sState, sZipCode, sPhone, sFax, sHours) Then
	If sPayMethod = "CASH" Or sPayMethod = "CHECK" Then
		If SetPaymentInfo(nOrderID, sPayMethod, dTip, "") Then
				...
				If Len(Session("PROMOCODES")) > 0 Then
					For i = 0 To UBound(naPromoCodes)
						If naPromoCodes(i) = "FREESB" Then
							sPromoDisplay = sPromoDisplay & "Free Small Salad and Small Cheezy Breadsticks With Any Pizza<br />"
						Else
							...
							Else
								If naPromoCodes(i) = "FREESB2" Then
									sPromoDisplay = sPromoDisplay & "Free Small Salad, Small Cheezy Breadsticks and 2 Liter With Any Pizza<br />"
								Else
			...
			Else
				If InStr(UCase(Request.ServerVariables("URL")), "ORDERTESTNEW") = 0 And InStr(UCase(Request.ServerVariables("SERVER_NAME")), "ARLGLOBAL.COM") = 0 Then
					If GetActivityOrderID(Session("ACTIVITYID")) <> -1 Then
						SendMail "vito@vitos.com", "4192975309@vtext.com, 4193203567@vtext.com, 4192606748@vtext.com, 4193672798@vtext.com", "", "", "Store Down", "Store " & nStore & " failed to print order #" & nOrderID, FALSE
...
%>
					<b>We apologize but your order could not be processed (<%=sResult%>).<br />
...
<%
		End If
	Else
		If SetTip(nOrderID, dTip) Then
%>
					<b>You will now be redirected to a secure payment 
					gateway to enter your credit card details. Once payment is 
approved your order will be sent to our store and you will receive an order 
confirmation.</b><br />&nbsp;<br />
			        <form id="ProcessForm" method="post" action="<%=gs_PaymentGWURL%>">
			            <input type="hidden" name="ClientSessionID" value="<%=nOrderID%>" />
			            <input type="hidden" name="ProcessType" value="CreditCard" />
			            <input type="hidden" name="TransType" value="Auth" />
			            <input type="hidden" name="UserName" value="<%=GetPGWID(nStore)%>" />
			            <input type="hidden" name="Amount" value="<%=dTotal%>" />
			            <input type="hidden" name="TransNum" value="<%=nOrderID%>" />
			            <input type="hidden" name="ReturnURL" value="<%=gs_PaymentGWReturnURL%>" />
			            <input type="hidden" name="HasHeader" value="false" />
			            <input type="hidden" name="NameOnCard" value="<%=Session("FIRSTNAME")%>&nbsp;<%=Session("LASTNAME")%>" />
			            <input type="hidden" name="Address" value="<%=sDisplayAddress%>" />
			            <input type="hidden" name="City" value="<%=Session("CITY")%>" />
			            <input type="hidden" name="State" value="<%=Session("STATE")%>" />
			            <input type="hidden" name="Zip" value="<%=Session("ZIPCODE")%>" />
			            ...
			            <input type="hidden" name="PromptCancel" value="" />
			            <input type="hidden" name="ShowTransNo" value="" />
			            <input type="hidden" name="CanSaveCard" value="No" />
			            <input type="hidden" name="PONum" value="<%=nOrderID%>" />
			            <input type="hidden" name="InvoiceNo" value="<%=nOrderID%>" />
			            ...
			            <input type="hidden" name="CustomerStreet2" value="" />
			            ...
			            <input name="submit" type="image" src="images/submit.jpg" />
			        </form>
<%
		Else
%>
