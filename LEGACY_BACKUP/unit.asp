...
Dim sPromoCodeError, nUID, nWidth, nHeight, nDepth, sType, sCurUnit, sCurDesc
...
If Request("uid").Count = 0 Or Not IsNumeric(Request("uid")) Then
...
If Len(saUnitName(0)) > 0 Then
	For i = 0 To UBound(saUnitName)
		If naUnitNum(i) = nUID Then
			sCurUnit = saUnitName(i)
			sCurDesc = saCustomDesc(i)
			Exit For
		End If
	Next
	...
	sFileName = "images/unit-titles/unit" & nUID & ".gif"
...
%>
				<tr>
					...
					<td valign="top" width="419">
						<img src="<%=sFileName%>" width="<%=nWidth%>" height="<%=nHeight%>" alt="<%=sCurUnit%>" />
					</td>
				</tr>
<%
		Else
%>
				<tr>
					<td valign="top" width="15"></td>
					<td valign="top" width="419"><%=sCurUnit%></td>
				</tr>
<%
		End If
	Else
...
%>
				...
				<tr>
					...
					<td valign="top" width="419">
<%
Dim saSpecial(), saSpecialDesc(), saSpecialID(), nColumn
...
<%
		...
		sFileName = "images/special-thumbs/" & nUID & ".gif"
		If oFS.FileExists(Server.MapPath(sFileName)) Then
			If gfxSpex(Server.MapPath(sFileName), nWidth, nHeight, nDepth, sType) Then
...
%>
							<tr>
								...
								<a href="customize.asp?uid=<%=nUID%>"><img src="<%=sFileName%>" alt="Create Your Own" /></a></td>
<%
			Else
%>
							<tr>
								<td valign="top" width="212">
								<a href="customize.asp?uid=<%=nUID%>">Create Your Own</a><br />
								<%=sCurDesc%></td>
<%
			End If
		Else
...
<%
			...
			sFileName = "images/special-thumbs/" & nUID & "-" & saSpecialID(i) & ".gif"
			If oFS.FileExists(Server.MapPath(sFileName)) Then
				If gfxSpex(Server.MapPath(sFileName), nWidth, nHeight, nDepth, sType) Then
					' TODO NOTE: FOR WHATEVER REASON RETRIEVING THE SIZE PERIODICALLY FAILS SO I'VE REMOVED IT FOR NOW
%>
								<td valign="top" width="212">
								<a href="customize.asp?uid=<%=nUID%>&spec=<%=saSpecialID(i)%>"><img src="<%=sFileName%>" alt="<%=saSpecial(i)%>" /></a></td>
<%
				Else
%>
								<td valign="top" width="212">
								<a href="customize.asp?uid=<%=nUID%>&spec=<%=saSpecialID(i)%>"><%=saSpecial(i)%></a><br />
								<%=saSpecialDesc(i)%></td>
<%
				End If
			Else
